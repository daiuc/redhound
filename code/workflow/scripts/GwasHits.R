...
# Adjust borders of overlapping loci to avoid overlaps -------------------------


olap.analysis = map(1:length(loci),
    ~ findOverlapPairs(loci[.x], loci[-c(.x)], minoverlap=1000))

# Loci NOT overlapping with any other locus
loci.nolaps = loci[lengths(olap.analysis) == 0]

# Loci that overlaps with other locus
loci.olaps = loci[lengths(olap.analysis) > 0]
# revert from region back to a single snp
loci.olaps.snps = resize(loci.olaps, width = 1) %>%
    GenomicRanges::shift(as.integer(locusSize/2))

cat(paste0("Of which ",
             length(loci.nolaps), " do not overlap; ",
             length(loci.olaps), " overlap. \nRedraw borders on overlapping loci."))

# merge overlapping regions to form a set of non-overlapping GR objects
merged.olaps = GenomicRanges::reduce(loci.olaps)

# for each of such GR, get SNPs that are in the range
grouped.olap.snps = map(1:length(merged.olaps),
                   ~findOverlapPairs(merged.olaps[.x],
                                     loci.olaps.snps,
                                     minoverlap = 1))
grouped.olap.snps = map(grouped.olap.snps, ~second(.x))

# overlapping loci are now redrawn to be non-overlapping
loci.olaps.redraw = map(grouped.olap.snps,
                        ~getNonOverlapLocus(.x))
loci.olaps.redraw = purrr::reduce(loci.olaps.redraw, c)

print(paste0("Successfully resized overlapping loci.",
             " Write out ", length(c(loci.nolaps, loci.olaps.redraw)),
             " GWAS hit loci to file."
             ))
...
c(loci.nolaps, loci.olaps.redraw) %>% sort %>%
    as.data.table(c(loci.nolaps, loci.olaps.redraw)) %>%
