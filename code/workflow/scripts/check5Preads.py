...
def readBED(file):
    ...
    # first line to determine if there's header
    with open(file) as f:
        ln1 = f.readline().split('\t')
        if ln1[0].isalnum() and ln1[1].isnumeric() and ln1[2].isnumeric():
            # bed format with no header
            ncols = len(ln1)
            if ncols > 6:
                colnames = ['chr', 'BEDstart', 'BEDend', 'name', 'score', 'strand'] + [f'V{str(x)}' for x in range(6, ncols, 1)]
            elif ncols == 6:
                colnames = ['chr', 'BEDstart', 'BEDend', 'name', 'score', 'strand']
            else:
                colnames = ['chr', 'BEDstart', 'BEDend']
            df = pd.read_csv(file, sep='\t', header=None, names=colnames)
        else:
            # bed format has header
...
def check5prime(ch, s, bam):
    ...
    try:
        ...
        if ref_base.upper() != ref_base: # get_aligned_pairs gives reference base in upper case, if there is a mismatch it will be in lower case
...
def Calc5PrimePercent(bed, bam): 
    '''
    bed: pd.Dataframe of RemapConsistency text output, include the map2_ad column
    ...
    for ch, st in zip(bed.chr, bed.BEDstart):
        a = len(set(check5prime(ch, st, bam).get('query_name'))) # get all reads that support the mismatch in 6bp of 5', compute how many of such reads
        b = bed[(bed['chr'] == ch) & (bed['BEDstart'] == st)]['map2_ad']
        b = list(b)[0] # number of reads supporting the mismatch
...
def main():
    ...
    parser.add_argument('--BED', type=str, required=True, help='txt file of editing sites, requires 8 columns') 
    ...
    parser.add_argument('--outTAB', type=str, required=True, help='output text file, incl all columns')
    ...
    # infer strand (only works with A>G or T>C)
    BED['strand'] = BED['name'].apply(lambda x: '+' if x == 'A_G' else ('-' if x == 'T_C' else '.'))
    ...
    BED[PASS_INDX].to_csv(args.outTAB, sep='\t', header=True, index=False)
    BED[PASS_INDX].loc[:, ['chr', 'BEDstart', 'BEDend', 'name', 'score', 'strand']
        ].to_csv(args.outBED, sep='\t', header=False, index=False)
