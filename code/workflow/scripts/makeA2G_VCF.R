...
if (exists("snakemake")) {
  ...
  input.files.ls = dir("Results/hs38/BCFCall/FinalAnno", "^.+[0-9]+\\.bed", full.names = T)
...
#     .[, ncol(.)] %>% str_replace_all("(JS\\-YZ\\-10S\\-)|(\\.bed)|(\\-)", "")
COLS = c("chr", "start", "end", "label",
         "af", "strand", "ref_ad", "alt_ad",
         "db", "gene", "feature", "repFam", "repCla")
chBED.ls = map(chBED.ls, ~ data.table::fread(.x,
    sep = "\t", header = F, col.names = COLS))
...
chBED.filtered.ls = map(chBED.ls, ~ filter(.x, af > af.cutoff & alt_ad > alt_ad.cutoff))

unique_sites = imap(chBED.filtered.ls,
                    function(df, df.nm) select(df, chr, start, end, label)) %>%
    do.call(bind_rows, .) %>%
    unique
...
label = str_split(unique_sites$label, "_", simplify = T)

vcf.out = data.frame(CHROM=unique_sites$chr, POS=unique_sites$start,
                     ID=".", REF=label[,1], ALT=label[,2], QUAL=".", FILTER="PASS",
                     INFO=".", GT="GT", "SPL"="0/1")
