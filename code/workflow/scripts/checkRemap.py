...
files: 
    1. a BED file of first pass putative editing sites. Result of "Gene_Features".
    2. a BAM file from the first alignment. Should be the same bam for bcftools callvariant. Result of MergeBamByPerson.
...
Qualified editing sites must satisfy two checks, implemented in functions checkMapping and checkSite:
    1. read names from first alignment bam file and the second star alignment bam file must be consistent.
    2. all putative sites must have at least 1 mismatch read counts detectable in the second alignment bam file.
...
def checkSite(chrom, start, mismatch, bamFile, min_edit = 1):
    """
    chrom: chr
    start: start 0 based
    mismatch: e.g. "A_G" or "T_G", this is column 4 of Results/hs38/BCFCall/FinalAnno/NA19130.bed
    ...
    if len(list(bamFile.fetch(chrom, start , end))) > 0: # must have alignments
        for r in bamFile.fetch(chrom, start, end):
            if r.has_tag('MD'): # make sure MD tag exisits
                ...
                if len(aligned_pair) > 0: # make sure get_aligned_pairs() return at least 1 result
                    ...
                    ref_query_base_pair.append("_".join((ref_base, query_base)))
        ...
        if len(ref_query_base_pair) > 0 and sum([pair in mismatch for pair in ref_query_base_pair]) > min_edit - 1: 
            # pass if there are at least min_edt number of AG or TC edits
            passCheck = True
            N_A2I = sum([pair in mismatch for pair in ref_query_base_pair])
...
def readBED(file):
    ...
    """
    import pandas as pd
    cols = ['chr', 'BEDstart', 'BEDend', 'name', 'score', 'strand']
...
def main():
    ...
    # first 6 cols of BED, note pysam use 0-based thus directly using BED pos
    ...
    i = 0
    keep_idx, agreement, ad = [], [], [] # ad is mismatch read counts
    for c, s, m in zip(BED['chr'], BED['BEDstart'], BED['name']):
        ck1, v1 = checkMapping(c, s, bam_pre, bam_pos)
        ck2, v2 = checkSite(c, s, m, bam_pos)
    ...
    out_df = BED.iloc[keep_idx]
    out_df.insert(6, 'agreement', agreement)
    out_df.insert(7, 'map2_ad', ad)
    # write output
