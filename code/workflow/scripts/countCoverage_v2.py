...
def combineCoverage(bamFile, bedFile):
    '''
    bam: bamFile path
    bed: 6 column bed file format
    return a dataframe with the first 6 columns identical to the bedFile
    ...
    bed = pd.read_csv(bedFile, sep="\t", 
                      names=['chr', 'BEDstart', 'BEDend', 'name', 'score', 'strand'],  
                      dtype={'chr': str, 'BEDstart': int, 'BEDend': int, 'name': str, 'score': int, 'strand': str})
